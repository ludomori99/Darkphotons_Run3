...
use_x509userproxy = True
x509userproxy = {HOME_USER}x509up_u238361
output = {MCRun3_logs_dir}job.$(Process).out
error = {MCRun3_logs_dir}job.$(Process).err
log = {MCRun3_logs_dir}job.$(Process).log
#RequestMemory = 4000
Universe = vanilla
queue {MCRun3_njobs}"""
with open(os.path.join(MCR3_FOLDER,"condor_MCRun3.sub"), "w") as file:
    file.write(template_MCRun3_condor)
