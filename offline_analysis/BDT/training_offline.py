...
class Trainer:
    ...
    def compute_reweight(data,variable,fitting_func, nbins_fit = 100, nbins_corrections=100, fit_range = None, fitting_limits=None, plot_reweight=False,saveas=None, nonnegative=True, xrange=(0,0.5), ylim=None, plot_logscale=False):
        ...
        if plot_reweight:
            ...
            ax.plot(x,fitting_func(x,*mData.values), label = "Non-prompt tail fit")
            ...
            ax.hist(data["Mm_kin_lxy"], nbins_corrections,  weights=weights, label="$J/\psi$ data reweighed", color = "red", zorder=0, histtype='step', linewidth = 1.8)
            ax.hist(lxylmDY[dimuonTriggerCut&lxycut], nbins_corrections,  weights=None, label="Low-mass DY samples (w/ dimuon trigger)", color = "blue", zorder=0, histtype='step', linewidth = 1.8)
